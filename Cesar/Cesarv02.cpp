...
string receptor::descifrar(string textoclaro){
    ...
    for (int i=0; i<largo; i++){//Evalua la cantidad de letras
        for (int j = 0; j < largo_alfabeto; j++)//cada una se compara con todo el alfabeto
        {
            if (textoclaro[i]==alfabeto[j])
            {
                aux2=j-clave;
...
string criptoanalisis(string mensaje){ //Usa descifrado
    string prueba, alfabeto="ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz1234567890-,.()";
    int aux, largo=mensaje.length(),aux2,key=1, confirmacion=0, largo_alfabeto=alfabeto.length();
